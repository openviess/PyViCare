...
class Vitocal300G_CU401B(unittest.TestCase):
    def setUp(self):
    ...
    # Load class tests - now work with fallback to statistics.load
    ...
    def test_getReturnTemperature(self):
        self.assertAlmostEqual(
            self.device.getReturnTemperature(), 35.8)
    ...
    def test_getReturnTemperaturePrimaryCircuit(self):
        self.assertAlmostEqual(
            self.device.getReturnTemperaturePrimaryCircuit(), 4.9)
    ...
    # Hot water storage top temperature test (uses fallback path)
